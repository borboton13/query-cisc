...
select (@folio := @folio + 1), '2020-11-16' as fecha, 'COMISION BANCO' as descripcion, 'PENDING' as estado, 5.00 as valor, 1 as idcompania, p.`idzonaproductiva`, d.`idproductormateriaprima`, 7 as idtipomovimientoproductor 
...
where p.fechainicio = '2020-11-16'
...
where p.`FECHA_ENTREGA` between '2020-11-01' and '2020-10-31'
...
-- set @folio = 15093; 
-- INSERT INTO sf_tmpdet (id_tmpdet, cuenta, no_cia, debe, haber, moneda, no_trans, id_tmpenc, idpersonacliente) 
select (@folio := @folio+1), '1421010100', '01', 0, p.`TOTALIMPORTE`, 'P', 153941, 154121, pe.`IDPERSONACLIENTE`    -- , P.`IDPEDIDOS` ,pe.`NOM`, pe.`AP`, pe.`AM`, p.`FECHA_ENTREGA` , per.`idpersona`
from pedidos p
join personacliente pe on p.`IDCLIENTE` = pe.`IDPERSONACLIENTE`
join entidad e         on pe.`NRO_DOC` = e.`noidentificacion`
join persona per       on e.`identidad` = per.`idpersona`
join productormateriaprima pr on e.`identidad` = pr.`idproductormateriaprima`
where p.`FECHA_ENTREGA` between '2020-10-01' and '2020-10-15'
and p.`ESTADO` <> 'ANULADO'
and p.`IDUSUARIO` <> 5
