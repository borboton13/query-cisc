...
select (@folio := @folio + 1), '2020-09-16' as fecha, 'COMISION BANCO' as descripcion, 'PENDING' as estado, 5.00 as valor, 1 as idcompania, p.`idzonaproductiva`, d.`idproductormateriaprima`, 7 as idtipomovimientoproductor 
...
join persona per       on e.`identidad` = per.`idpersona`
join productormateriaprima pr on e.`identidad` = pr.`idproductormateriaprima`
where p.`FECHA_ENTREGA` between '2020-09-01' and '2020-09-15'
and p.`ESTADO` <> 'ANULADO'
and p.`IDUSUARIO` <> 5
