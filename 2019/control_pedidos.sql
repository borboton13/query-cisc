...
-- AND p.`IDCLIENTE` = 65
and p.`CODIGO` in (5821, 5836) -- 3939(8895), 4140(119), 4216(347)
-- 3133
...
-- update pedidos p set p.`FECHA_ENTREGA` = '2019-08-27' where p.`IDPEDIDOS`= 34222; -- 2019-08-24
-- update pedidos p set p.`FECHA_ENTREGA` = '2019-08-27' where p.`IDPEDIDOS`= 34237; -- 2019-08-24
...
where m.`no_usr` = 'SUA'
and v.`cod_alm` = 2
and v.`fecha` between '2019-05-01' and '2019-02-28'
and d.`cod_art` in (118, 148, 150, 151, 643)
;

-- PEDIDOS FACTURAS
select p.`IDPEDIDOS`, p.codigo, p.`ESTADO`, concat(pc.`NOM`, ' ', pc.`AP`, ' ', pc.`AM`) as Nombre , p.`FECHA_ENTREGA` as fecha, p.`TOTAL`, p.`TOTALIMPORTE`, /*P.`IDMOVIMIENTO`,*/ m.`FECHA_FACTURA`, m.`NROFACTURA`, m.`RAZON_SOCIAL`, m.`IMPORTE_TOTAL`
from pedidos p
left join personacliente pc on p.`IDCLIENTE` = pc.`IDPERSONACLIENTE`
left join movimiento m on p.`IDMOVIMIENTO` = m.`IDMOVIMIENTO`
where p.`FECHA_ENTREGA` between '2018-05-01' and '2018-11-30'
-- and p.`ESTADO` <> 'ANULADO'
